...
def ingresar_datos() -> dict:
    ...
    nombre = input("Ingresa nombre de usuario: ").capitalize()
    contra = input("Ingresa contraseña: ").lower()
...
#Funcion que valida si un ususario existe actualmente
def validar_usuario(base_de_datos: list):
    nombre_porvalidar = input("Ingrese nombre de usuario: ").capitalize()
    for usuario in base_de_datos:
        if usuario["Nombre"] == nombre_porvalidar:
            contra = input("Ingrese contraseña: ").lower()
...
#funcion principal, conecta todas las demas funcionas y variables en conjunto
def main():
    ...
    while True:
        definir_direccion = input("Desea ingresar con su usuario (I) o registrar un usuario nuevo (R) o (S) para salir: ")
        if definir_direccion.upper() == "R":          
            print(f"Nuevo formulario N {formularios_cargados + 1}")
            formulario = ingresar_datos()
            guardar_datos(formulario,ruta_archivo,base_de_datos)
            print(f"Cantidad de registros en la base de datos: {formularios_cargados + 1}")

        elif definir_direccion.upper() == "I":
            validar_usuario(base_de_datos)

        elif definir_direccion.upper() == "S":
            print("*********************")
            print("Se cerro el programa")
            break
        
        else:       
            print("Porfavor ingrese un opcion valida")

        direccion2 = input("Desea continuar/repetir el programa (S) o (N): ")
        if direccion2.upper() == "N":
            print("*********************")
            print("Se cerro el programa")
            break
